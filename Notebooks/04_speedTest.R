...
greb = list ( greb , rb)
save( greb , file = file.path(PROJHOME , "Output" , "speedTest" , "data1.rda"))


rm(list= ls())
load ( file.path(PROJHOME , "Output" , "speedTest" , "data1.rda"))
dats = NULL#
for ( i in 1:200){
  vaL = 500 + i
...
ggplot ( ggitem, aes( x=pred.speed, y = effect.size.2 , col = as.character ( prop.ds) ))+
  geom_line()

save( ggitem, file=  file.path ( PROJHOME , "Plots" , "PlotData", "P01_speedTest1.rda"))
